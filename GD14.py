...
class PemutarMusik(Frame):
    def __init__(self,parent):
        ...
        self.nama = StringVar()
        self.nama.set("Radio Playlistmu")
    ...
    def buatOpen(self):
        tombol = Button(text="Playlist", command=self.bukaFile)
    ...
    def slider(self):
        w1 = Scale(from_=0.00, to=1.0,resolution=0.1, command=self.volume, orient=HORIZONTAL, length=500, label='Volume Music :',showvalue=0)
        w1.pack()
        w1.set(0.50)
    ...
    def getPosisi(self):
        ...
        if len(self.penampungFile) == 0 and self.b == False and self.diStop == False :
            self.nama.set("Selesai nih musiknya")
            self.a.set("Play")
